...
`str-raci-matrix.json` 为 STR-01—08 登记唯一 A、至少一个 R、验收证据、投诉路径、回滚、维护和 sunset 的治理入口；只使用通用角色，资源级别仅 L0—L2，不设 L3。每包新增规划成本带、容量假设、`D01—15 / D16—30 / D31—60 / D61—75 / D76—90` 五段里程碑与维护周期，使“谁负责、做多少、何时验收、失败怎样退出”可以逐项审查。[data:visual/assets/str-raci-matrix.json#packages]

| 包 | 规划成本带 | 容量假设（待核） | 90天控制重点 | 维护周期 | 验收、停止与回滚 |
| --- | ---: | --- | --- | --- | --- |
| STR-01 | 10—30 万元/包 | ≤300 条资料/轮；≤1 次版本评审/周 | 清权与参考条件 → 人工基线 → 校准/质疑/恢复演练 → 交叉复核 → 验收包 | 每月；官方资料变化即复核 | 验来源、校准、版本和人工判断；无参考条件只留教育展示，撤回未核输出并恢复上一核读版 |
| STR-02 | 30—50 万元/包 | ≤6 台/批、≤3 批/日；仅人工值守窗 | 空间/供电/网络前置 → 14天基线 → 共置/漂移/撤下演练 → 安全维护复核 → 人决去留 | 每批前校准；每周维护；异常即时停机 | 验授权、隔离、维护和静态替代；漂移、权利或运维缺位即停机撤下 |
| STR-03 | 20—50 万元/包 | ≤20 次辅助互动/值守窗；开放频次未承诺 | 同意/权利/责任人 → 等效服务基线 → 共译/借用/删除演练 → 无障碍复核 → 验收/停止 | 每个值守窗开闭场；每周内容与无障碍复核 | 验同意、培训、值守、纸质/语音/人工路径；无责任人则停止公共建议 |
| STR-04 | 50—80 万元/包 | ≤8 名自愿参与者/日、≤30 分钟/人 | 物业/机电/消防前置 → 10天人工基线 → 非公共沙盒 → 安全/隐私/撤场演练 → 人决去留 | 每日开场检查；每次会话记录；重大事件即时停测 | 验授权、终端可用性、投诉退出和非AI等效路径；任一失效即停测并撤回 |
| STR-05 | 20—40 万元/包 | 6 条接口/轮；1—2 条/审计日 | 冻结接口问题 → 合法安全通行基线 → 分批现场审计 → 专业复核 → 仅发布已核接口 | 每次审计前；道路/施工/权属变化即重审 | 验测绘、交通、无障碍、权属和绕行；不满足合法安全通行即撤回关系线并绕行 |
| STR-06 | 40—80 万元/包 | 12 原型；单点同时≤3组件，逐项人工闸 | 组件/权利/维护登记 → 撤下基线 → ≤3代表组件装配演练 → 触发器复核 → 逐项去留 | 每周巡检；每次活动前权利/安全复核 | 验组件、许可、人工替代和投诉路径；超期、失准、权利或维护失效即停用撤下 |
| STR-07 | 10—30 万元/包 | ≤20 个双语模块/轮；未核项不发布 | 史实/版权/译校规则 → 双语/易读基线 → 首批静态原型 → 多角色复核 → 限定发布 | 每月史实/权利/译文复核；每季度归档 | 验来源、权利、译校和可访问性；不能确认即留白、暂停或撤回旧版 |
| STR-08 | 20—40 万元/包 | ≤1 次完整版本复盘/季度；无SLA承诺 | 质疑/责任/停止规则 → 人工接收基线 → 问题处置/回滚演练 → 双语恢复复核 → 授权后开放 | 每月核台账；每季度恢复演练 | 验版本、责任、投诉、回滚和归档；无运营主体则保持内部台账，不对外承诺 |

上述成本、容量、服务窗口和维护周期全部是**规划级假设、非报价、非预算批复、非伙伴承诺、非场地授权、非 SLA**。季度校准与场景复盘、半年模型/数据审查、年度开放挑战仍只是可审查机制；真实责任人、资源、专业条件和候选级 `HumanDecision` 未完成前，不进入公开试点。[data:visual/assets/str-raci-matrix.json#implementation_control_boundary]
